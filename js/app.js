  ...
  renderHome() {
    ...
    if (homeLectureBox && upcomingLecture) {
      homeLectureBox.innerHTML = `
        <div style="font-size: 13px; font-weight: 700; color: #ef4444; margin-bottom: 4px;">WEEK ${upcomingLecture.week} (다음 진행 예정 강의)</div>
        <h4 style="font-size: 17.5px; margin-bottom: 10px; font-weight: 700; color: #fff; line-height: 1.35;">${upcomingLecture.title}</h4>
        <p style="font-size: 13px; color: var(--color-on-dark-mute); margin-bottom: 12px; border-bottom: 1px dashed var(--color-hairline-strong); padding-bottom: 10px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="📅 ${upcomingLecture.date} | 📍 ${upcomingLecture.location}">📅 ${upcomingLecture.date} | 📍 ${upcomingLecture.location}</p>
  ...
  renderLedger() {
    ...
    const initialBalanceRowHtml = `
      <tr style="background: rgba(99, 102, 241, 0.05); font-weight: 600; border-top: 2px solid var(--color-hairline);">
        ...
        <td style="font-weight: 800; color: #1e1e2e; white-space: nowrap; text-align: right; background: rgba(99, 102, 241, 0.08);">
          ${initBalance.toLocaleString()}원
        </td>
    ...
    const rowsHtml = filteredLedger.map(item => {
      ...
      return `
        <tr>
          ...
          <td style="font-weight: 700; color: ${isIncome ? '#16a34a' : '#dc2626'}; white-space: nowrap; text-align: right;">
            ${isIncome ? '+' : '-'}${amt.toLocaleString()}원
          </td>
          <td style="font-weight: 700; color: var(--color-ink); white-space: nowrap; text-align: right; background: rgba(0,0,0,0.015);">
